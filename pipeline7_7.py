# PIPELINE 7.7 – CORE-ONLY REVERSE + ESCAPE-PREVENTION + CRITICAL EVENT LOG
# All 7.6 features kept + your exact requests

import os
import numpy as np
import matplotlib.pyplot as plt

OUTPUT_DIR = "pipeline_7_7_core_escape_critical"
...
r0 = 10.0
THRUST_BASE = 0.035
ESCAPE_BOOST = 0.12          # extra inward kick if escaping
np.random.seed(42)

# =============================================================
# INLINE ODIM + QSTF (same as 7.6)
# =============================================================
...
def continue_post_throat(mode, r_start, step_offset, odim_f, region_f, event_f):
    ...
    for i in range(MAX):
        ...
        # Bubble + ODIM feedback
        bubble_active = abs(r) < 8.0
        odim_feedback = 1.0 + 8.0 * proxy_ent
        bubble_boost = -0.003 * r * odim_feedback if bubble_active else 0.0

        # === STRICTER REVERSE TRIGGER (core only) ===
        if (proxy_ent > 5e6 and abs(r) < 0.001 and not reversed_polarity):
            reversed_polarity = True
            reverse_timer = 3000
            event_f.write(f"[{mode}] Step {step}: REVERSE PLATES TRIGGERED AT CORE (proxy={proxy_ent:.2e}, r={r:.3e})\n")

        # === ESCAPE-PREVENTION THRUSTERS ===
        velocity = (r - last_r) / delta_tau
        if velocity > 0.0 and r < 0.0:  # trying to escape outward
            escape_boost = -ESCAPE_BOOST * abs(r) * (1.0 + proxy_ent)
            event_f.write(f"[{mode}] Step {step}: ESCAPE VELOCITY DETECTED – THRUST BOOST FIRED (vel={velocity:.4f})\n")
        else:
            escape_boost = 0.0
        ...
        r += (dr_dtau + thrust + quantum + bubble_boost + escape_boost) * delta_tau + osc + extra_in

        # White-hole (unchanged)
        ...
        if i % 500 == 0 or reversed_polarity or bubble_active or (velocity > 0 and r < 0):
        ...
        if i % 500000 == 0 and i > 0:
            print(f"[{mode}] Step {step}: r={r:.4e}  curv={curv:.4e}  reversed={reversed_polarity}")

    return np.array(plot_steps), np.array(plot_r), np.array(plot_d), np.array(plot_f), np.array(plot_c)

def main():
    print("Pipeline 7.7 – Core-Only Reverse + Escape-Prevention Thrusters + Critical Event Log")
    print("Stricter core trigger + auto-boost if escaping + full event recording")
    pre_log, r_throat = run_pre_throat()

    with open(os.path.join(OUTPUT_DIR, "odim_evolution.txt"), "w", encoding="utf-8") as odim_f, \
         open(os.path.join(OUTPUT_DIR, "region_log_7_7.txt"), "w", encoding="utf-8") as region_f, \
         open(os.path.join(OUTPUT_DIR, "critical_events_7_7.txt"), "w", encoding="utf-8") as event_f:

        for line in pre_log: region_f.write(line + "\n")

        for mode in ["A", "B", "C"]:
            print(f"\nRunning {mode} with core-only reverse + escape prevention...")
            steps, r, d, f, c = continue_post_throat(mode, r_throat, 40000, odim_f, region_f, event_f)

            plt.figure(figsize=(10,5))
            plt.plot(steps, r, label=mode)
            plt.title(f"Radius {mode} – 7.7 Core + Escape Prevention")
            plt.xlabel("Step")
    ...
    print("\n=== 7.7 COMPLETE ===")
    print("Check critical_events_7_7.txt for every dramatic moment we almost missed!")
