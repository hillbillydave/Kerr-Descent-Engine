# PIPELINE 7.4 – THRUSTER-ASSISTED INFALL (constant push + gravity)
# White-hole exit only if math strongly decides it

import os
import numpy as np
import matplotlib.pyplot as plt

OUTPUT_DIR = "pipeline_7_4_thrusters"
...
evap_rate = 1e-6
r0 = 10.0
THRUST = 0.025          # <-- change this if you want stronger/weaker push
np.random.seed(42)
...
def continue_post_throat(mode, r_start, step_offset, odim_f, region_f):
    ...
    for i in range(MAX):
        ...
        # THRUSTERS: constant outward push (ship-style)
        thrust = THRUST if not wh_active else -THRUST

        r += (dr_dtau + thrust + quantum) * delta_tau + osc + extra_in
        ...
        # Write directly to disk (no memory buildup)
        odim_f.write(f"{mode} {step:8d} | r={r:14.6e} | tau={step*delta_tau:12.4f} | "
                     f"proxy_ent={proxy:12.4e} | curv={curv:12.4e}\n")
...
def main():
    ...
    print("\nFinished! Check region_log_7_4.txt for WHITE HOLE or SINGULARITY messages.")
    print("Plots and logs are in:", OUTPUT_DIR)
