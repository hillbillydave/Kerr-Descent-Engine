# PIPELINE 7.3 – FORCE DEEP PLUNGE – WHITE HOLE ONLY IF MATH DECIDES
# No oscillation after horizon + stronger natural inward pull

import os
import numpy as np
import matplotlib.pyplot as plt

OUTPUT_DIR = "pipeline_7_3_deep_plunge"
...
# Post-throat – deep plunge version
def continue_post_throat(mode, r_start, step_offset, odim_f, region_f):
    ...
    for i in range(MAX):
        ...
        # === ALMOST NO OSCILLATION AFTER HORIZON ===
        osc = 0.00001 * np.sin(0.000005 * step) if abs(r) > 5 else 0.0

        # Stronger natural inward bias when deep
        ...
        # White-hole trigger (only if physics screams for it)
        proxy = 0.0
        if abs(r) < 0.005:
        ...
        odim_f.write(f"{mode} {step:8d} | r={r:14.6e} | τ={step*delta_tau:12.4f} | proxy_ent={proxy:12.4e} | curv={curv:12.4e}\n")

        if i % 500 == 0 or wh_active:
        ...
        if abs(r) < 1e-6 or curv > 1e24:
            region_f.write(f"[{mode}] Step {step}: SINGULARITY REACHED (r={r:.3e})\n")
            break

        if i % 500000 == 0 and i > 0:
            print(f"[{mode}] Step {step}: r={r:.4e} curv={curv:.4e} WH={wh_active}")

    return np.array(plot_steps), np.array(plot_r), np.array(plot_d), np.array(plot_f), np.array(plot_c)

def main():
    print("Pipeline 7.3 – Deep Plunge (white hole only if math decides)")
    pre_log, r_throat = run_pre_throat()

    with open(os.path.join(OUTPUT_DIR, "odim_evolution.txt"), "w") as odim_f, \
         open(os.path.join(OUTPUT_DIR, "region_log_7_3.txt"), "w") as region_f:
        ...
        for mode in ["A", "B", "C"]:
            ...
            # quick plot for this mode
            plt.figure(figsize=(10,5))
            plt.plot(steps, r, label=mode)
            plt.title(f"Radius {mode} – 7.3 Deep Plunge")
            plt.xlabel("Step")
    ...
    print("Done! Check region_log_7_3.txt for WHITE HOLE or SINGULARITY messages.")
